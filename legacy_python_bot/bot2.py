...
# Global product cache to avoid repeated cheapest product detection across dummys/batches
...
# ====== Core dummy check (bot-friendly) ======
def check_single_dummy(dummy: Dict, sites: List[str], proxies_override: Optional[Dict[str, str]] = None, runner=None) -> Tuple[str, str, str, str, Optional[str], Optional[str]]:
    # Track last exception message to propagate proxy/connect errors back to caller
    last_exception_msg: Optional[str] = None
    try:
        # Make a local copy merged with baseline dummy_DATA
    ...
    # Distribute starting site per dummy using a rotation offset derived from PAN
    try:
        ...
        try:
            ...
            if not receipt_id:
                ...
                if status == "unknown":
                    ...
                    continue
                # For terminal/non-unknown submit codes (e.g., PAYMENTS_CREDIT_dummy_NUMBER_INVALID_FORMAT),
...
# ====== Batch runner ======
class BatchRunner:
    def __init__(self, dummys: List[Dict], sites: List[str], executor: ThreadPoolExecutor, batch_id: str, chat_id: int, user_id: int, cancel_event: asyncio.Event, send_approved_notifications: bool = True, proxies_override: Optional[Dict[str, str]] = None, start_from: int = 0):
        self.dummys = dummys
        self.sites = sites
        self.executor = executor
        self.total = len(dummys)
    ...
    async def run(self, update: Update, context: ContextTypes.DEFAULT_TYPE, title: str):
        ...
        for fut in as_completed(futures):
            ...
            if status in ("approved", "charged") and self.send_approved_notifications:
                try:
                    notify_text = result_notify_text(dummy, status, code_display, amount_display, site_label)
    ...
    async def run_with_notifications(self, update: Update, context: ContextTypes.DEFAULT_TYPE, title: str):
        ...
        async def run_one(dummy: Dict):
            async with sem:
                ...
                try:
                    ...
                    result = await loop.run_in_executor(self.executor, check_single_dummy, dummy, self.sites, mapping_to_use)
                    ...
                    if len(result) == 6:
                        ...
                        used_proxy_url = selected_proxy_url
                        site_url = None
                    return dummy, status, code_display, amount_display, site_label, used_proxy_url, site_url
                except Exception as e:
                    try:
                        logger.warning(f"Batch task failed: {e}")
                    except Exception:
                        pass
                    return dummy, "unknown", '"code": "UNKNOWN"', "$0", "", selected_proxy_url, None

        # Create asyncio tasks for all dummys; semaphore enforces the per-batch concurrency cap
        ...
        for t in asyncio.as_completed(tasks):
            ...
            if self.cancel_event.is_set():
                ...
                break
            try:
                dummy, status, code_display, amount_display, site_label, used_proxy_url, site_url = await t
        ...
        if status in ("approved", "charged"):
            ...
            try:
                ...
                pass
            # Always send notifications for Charged dummys
            # For Approved dummys, send them regardless of user preference to ensure all approved dummys are shown in chat
            send_chat = (status == "charged") or (status == "approved")
            if send_chat:
                # Use the already fetched display name for notifications
                try:
                    await update.effective_chat.send_message(
                        text=result_notify_text(dummy, status, code_display, amount_display, site_label, display_name),
                        parse_mode=ParseMode.HTML,
                        disable_web_page_preview=True,
                    )
                except Exception as e:
...
SMALL_BATCH_THRESHOLD = 6  # <= this many dummys considered a small/interactive batch
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "• /st <dummy> — Check a single or multiple credit dummys (max 25)\n"
...
async def cmd_mass(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Support inline proxy definition at the top; remove it from the body before parsing dummys
    ...
    # Enforce maximum of 100 dummys for /sh
...
async def cmd_setpr(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for ln in raw_lines:
        ...
        if low.startswith("proxy=") or low.startswith("proxy:") or low.startswith("px=") or low.startswith("px:") or low.startswith("px "):
            ...
            continue
        # Treat CC-like lines as dummy input (first one only)
        maybe_dummy = None
        try:
    ...
    # Use multiple test dummys for better validation
    test_dummys = [
        {
    ...
    # Validate each proxy; consider valid if it works with any dummy on any site
    ...
    if not items:
        ...
        for pbid, payload in list(pend.items())[:10]:
            try:
                ...
                dummys = payload.get("dummys") or []
                sites = payload.get("sites") or []
                lines.append(f"• {title} — dummys: {len(dummys)} — Sites: {len(sites)}")
            except Exception:
                continue
        await update.message.reply_text("\n".join(lines), disable_web_page_preview=True)
        return
...
async def cmd_st_cc(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Get dummys from command argument or reply
    full_text = (update.message.text or "").strip()
    dummys_text = ""
    
    # Try to extract dummys from command argument
    try:
    ...
    # If no dummys in command, try to get from replied message
    if not dummys_text:
        replied = update.message.reply_to_message
        if replied and isinstance(getattr(replied, "text", None), str) and replied.text.strip():
    ...
    if not dummys_text:
        await update.message.reply_text(
            "Usage:\n"
            "/st <dummy> - Check a single dummy\n"
            "/st <dummy1>\n<dummy2>\n... - Check multiple dummys (max 25)\n"
            "Example: /st 4242424242424242|12|25|123"
        )
        return
    
    # Parse dummys from text (one per line)
    ...
    # Convert dummy lines into dummy dictionaries
    dummys = []
