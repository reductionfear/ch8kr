...
def parse_dummys_from_text(text: str) -> List[Dict]:
    """Optimized dummy parsing with fast path for well-formatted dummys"""
    import neww as checkout
    import re
    
    dummys = []
    seen_dummys = set()  # Track dummy numbers we've already found
    text = text or ""
    
    # OPTIMIZATION: Fast path for well-formatted dummys (most common case)
    ...
    for match in fast_matches:
        try:
            ...
            if 1 <= month <= 12:
                ...
                dummy_key = f"{number}|{month}|{year}|{cvv}"
    ...
    # If fast path found many dummys, we're done (no need for slower parsing)
    if len(dummys) >= 10:
        return dummys
    
    # Fallback: line-by-line extraction for other formats
    for line in text.splitlines():
        dummy = checkout.parse_cc_line(line)
        if dummy:
            dummy_key = f"{dummy['number']}|{dummy['month']}|{dummy['year']}|{dummy['verification_value']}"
            if dummy_key not in seen_dummys:
                seen_dummys.add(dummy_key)
                dummys.append(dummy)
    
    # Only use multi-line extraction as last resort if we found very few dummys
    # This handles cases where dummy data is spread across multiple lines
    if len(dummys) < 5:  # If we found fewer than 5 dummys, try aggressive extraction
        multi_line_dummys = _extract_multiline_dummys(text)
        for dummy in multi_line_dummys:
            dummy_key = f"{dummy['number']}|{dummy['month']}|{dummy['year']}|{dummy['verification_value']}"
            if dummy_key not in seen_dummys:
                seen_dummys.add(dummy_key)
                dummys.append(dummy)
...
def _extract_multiline_dummys(text: str) -> List[Dict]:
    """Extract dummy information that spans multiple lines with labels
    
    Can extract dummys even when mixed with junk text like:
    ...
    # Pattern to find dummy numbers with various labels
    # dummy numbers are 13-19 digits (commonly 16)
    dummy_patterns = [
        r'(?:ccnum|cc num|dummy num|dummy number|dummy|cc|number|pan|dummynumber)\s*[:=\-]?\s*(\d{13,19})',
        r'\b(\d{13,19})\b',  # Standalone dummy number
    ...
    # Try to find all dummy numbers first
    dummy_numbers = []
    seen_dummy_nums = {}  # Track unique dummy numbers with their first occurrence position
    for pattern in dummy_patterns:
        matches = re.finditer(pattern, text, re.IGNORECASE)
        for match in matches:
            dummy_num = match.group(1)
            # Validate it's a proper dummy number length
            if 13 <= len(dummy_num) <= 19:
                # Only add if we haven't seen this exact dummy number before
                if dummy_num not in seen_dummy_nums:
                    seen_dummy_nums[dummy_num] = (match.start(), match.end())
                    dummy_numbers.append((dummy_num, match.start(), match.end()))
    
    # For each dummy number found, try to find nearby exp and cvv
    for dummy_num, dummy_start, dummy_end in dummy_numbers:
        # Look in a window around the dummy number (200 chars before and 300 after)
        # This helps avoid picking up data from other dummys
        window_start = max(0, dummy_start - 200)
        window_end = min(len(text), dummy_end + 300)
        window_text = text[window_start:window_end]
        
        # Calculate dummy number position in window (used for filtering)
        dummy_offset_in_window = dummy_start - window_start
        dummy_in_window_start = dummy_offset_in_window
        dummy_in_window_end = dummy_offset_in_window + len(dummy_num)
        ...
        if not exp_month or not exp_year:
            ...
            for month_match in two_digit_matches:
                # Skip if this match overlaps with the dummy number position
                if (month_match.start() >= dummy_in_window_start and 
                    month_match.start() < dummy_in_window_end):
                    continue
                ...
                if 1 <= potential_month <= 12:  # Valid month
                    ...
                    for year_match in year_matches:
                        # Skip if this overlaps with dummy number position
                        if (year_match.start() >= dummy_in_window_start and 
                            year_match.start() < dummy_in_window_end):
                            continue
        ...
        # Find CVV - prefer CVVs that appear AFTER the dummy number
        cvv = None
        ...
        # Filter out dummy number, month, year from potential CVVs
        valid_3digit = []
        for match in all_3digit:
            # Skip if this overlaps with dummy number position
            if (match.start() >= dummy_in_window_start and 
                match.start() < dummy_in_window_end):
                continue
        ...
        if len(valid_3digit) == 1:
            ...
            for i, pattern in enumerate(cvv_patterns):
                ...
                # Sort matches by proximity to dummy (prefer those after the dummy)
                matches_after = [m for m in matches if m.start() > dummy_offset_in_window]
                matches_before = [m for m in matches if m.start() <= dummy_offset_in_window]
                sorted_matches = matches_after + matches_before
                
                for match in sorted_matches:
                    potential_cvv = match.group(1)
                    # Don't use the dummy number, exp date, month, or year as CVV
                    is_valid_cvv = (potential_cvv != dummy_num[-4:] and potential_cvv not in dummy_num)
                    if exp_year:
                    ...
                    if is_valid_cvv:
                        ...
                        if i == len(cvv_patterns) - 1:
                            ...
                            if potential_cvv not in dummy_num:
                                cvv = potential_cvv
                                break
                        else:
                            cvv = potential_cvv
        ...
        # If we found all components, create a dummy
    ...
    # Deduplicate dummys before returning
    unique_dummys = []
    seen = set()
    for dummy in dummys:
        dummy_key = f"{dummy['number']}|{dummy['month']}|{dummy['year']}|{dummy['verification_value']}"
        if dummy_key not in seen:
            seen.add(dummy_key)
            unique_dummys.append(dummy)
    
    return unique_dummys

def parse_dummys_from_file(file_path: str) -> List[Dict]:
    """Parse dummys from file with optimizations for large files"""
    try:
        with open(file_path, "r", encoding="utf-8", buffering=8192) as f:
            content = f.read()
        return parse_dummys_from_text(content)
    except Exception:
        return []
...
def classify_prefix(code_display: str) -> str:
    ...
    approved_tokens = (
        ...
        "SECURITY",
        "VERIFICATION",
        "PAYMENTS_CREDIT_dummy_CVV_INVALID",
        "PAYMENTS_CREDIT_dummy_VERIFICATION_VALUE_INVALID",
        "PAYMENTS_CREDIT_dummy_CSC_INVALID",
        "PAYMENTS_CREDIT_dummy_SECURITY_CODE_INVALID",
    )
...
def result_notify_text(dummy: Dict, status: str, code_display: str, amount_display: Optional[str] = None, site_label: Optional[str] = None, user_info: Optional[str] = None, receipt_id: Optional[str] = None, user_id: Optional[int] = None) -> str:
    pan = str(dummy.get("number", "") or "")
    mm = int(dummy.get("month", 0) or 0)
    yy = int(dummy.get("year", 0) or 0)
    cvv = str(dummy.get("verification_value", "") or "")
    mm_str = f"{mm:02d}"
    yy_str = f"{yy % 100:02d}"
    ...
    parts = [
        ...
        "━━━━━━━━━━━━━━━━━━━━━━━━━━━━",
        f"💳 <b>dummy:</b> <code>{pan}|{mm_str}|{yy_str}|{cvv}</code>"
    ]
    ...
    # Add receipt ID for approved and charged dummys
