...
BOT_TOKEN = "PUT YOUR BOT_TOKEN HERE"
...
async def resume_pending_batches(app):
    ...
    for batch_id, payload in pend.items():
        try:
            ...
            dummys = payload.get("dummys") or []
            sites = payload.get("sites") or []
            send_approved = bool(payload.get("send_approved_notifications", True))
            if not isinstance(dummys, list) or not isinstance(sites, list) or not chat_id or not user_id:
                continue
            ...
            processed = 0
            original_total = len(dummys) if isinstance(dummys, list) else 0
            try:
                processed = int(payload.get("processed", 0))
                if processed > 0 and isinstance(dummys, list):
                    # Slice dummys to only include remaining ones, but preserve original_total for progress display
                    dummys = dummys[processed:]
            except Exception:
                processed = 0
            ...
            try:
                chosen_executor = GLOBAL_EXECUTOR if (isinstance(dummys, list) and len(dummys) > SMALL_BATCH_THRESHOLD) else SMALL_TASK_EXECUTOR
            except Exception:
                chosen_executor = GLOBAL_EXECUTOR
            # Pass total_override to preserve the original total count when resuming
            runner = BatchRunner(dummys, sites, chosen_executor, resumed_batch_id, chat_id, user_id, cancel_event, 
...
from bot1 import ensure_uploads_dir, _sanitize_filename_component, _username_prefix_for_file, parse_dummys_from_text, parse_dummys_from_file, progress_block, format_site_label, classify_prefix, result_notify_text
...
def check_single_dummy(dummy: Dict, sites: List[str], proxies_override: Optional[Dict[str, str]] = None, runner=None) -> Tuple[str, str, str, str, Optional[str], Optional[str], Optional[str]]:
    last_exception_msg: Optional[str] = None
    try:
        dummy_data = {**checkout.dummy_DATA, **dummy}
    except Exception:
        dummy_data = dummy
    ...
    try:
        pan_digits = "".join([c for c in str(dummy_data.get("number", "")) if c.isdigit()])
    except Exception:
        pan_digits = ""
    ...
    for site_idx, site in enumerate(ordered_sites):
        ...
        try:
            ...
            dummy_session_id = checkout.step2_tokenize_dummy_ctx(session, checkout_token, shop_url, dummy_data)
            ...
            queue_token, shipping_handle, merchandise_id, actual_total, delivery_expectations, phone_required = checkout.step3_proposal_ctx(
                session, checkout_token, session_token, dummy_session_id, shop_url, variant_id
            )
            ...
            print(f"[DEBUG] Input params - dummy_session_id: {dummy_session_id[:30] if dummy_session_id else 'None'}...")
            ...
            try:
                receipt_result = checkout.step4_submit_completion_ctx(
                    ...
                    shipping_handle, merchandise_id, dummy_session_id,
            ...
            if not receipt_id:
                ...
                if "PAYMENTS_CREDIT_dummy_BRAND_NOT_SUPPORTED" in submit_upper:
                    try:
                        code_display = f'"code": "PAYMENTS_CREDIT_dummy_BRAND_NOT_SUPPORTED"'
                    except Exception:
                        code_display = '"code": "PAYMENTS_CREDIT_dummy_BRAND_NOT_SUPPORTED"'
                    status = "declined"
                ...
                site_level_submit_errors = (
                    ...
                    "VALIDATION_CUSTOM",
                    "VALIDATION_",
                    "PAYMENTS_CREDIT_dummy_SESSION_ID",
                ...
                if is_site_level_error or no_receipt_id_site_error:
                    ...
                    # For normal dummy checking: continue to next site
                    try:
                        with BOT_PRODUCT_CACHE_LOCK:
                            if shop_url in site_product_cache:
                                del site_product_cache[shop_url]
                    ...
                    print(f"[DEBUG] Step 4 - Continuing to next site for same dummy")
                    continue
                try:
            ...
            try:
                ...
                if "PAYMENTS_CREDIT_dummy_BRAND_NOT_SUPPORTED" in code_upper:
                    status = "declined"
                    return status, code_display, _amount_display(), site_label, used_proxy_url, shop_url, receipt_id
            except Exception:
                pass
            ...
            # Track site health - mark as success if dummy was processed
            try:
                if status in ("approved", "declined", "charged"):
                    ...
                    # For other statuses, still count as success (dummy was checked)
                    update_site_health(shop_url, "success")
            except Exception as track_err:
                logger.debug(f"Failed to track site health: {track_err}")
...
class BatchRunner:
    def __init__(self, dummys: List[Dict], sites: List[str], executor: ThreadPoolExecutor, batch_id: str, chat_id: int, user_id: int, cancel_event: asyncio.Event, send_approved_notifications: bool = True, proxies_override: Optional[Dict[str, str]] = None, start_from: int = 0, total_override: Optional[int] = None):
        self.dummys = dummys
        self.sites = sites  # Use sites as-is, optimization done by caller if needed
        self.executor = executor
        # If total_override is provided (e.g., when resuming), use it instead of len(dummys)
        # This fixes the bug where resumed batches show wrong total (e.g., 273/127 instead of 273/400)
        self.total = total_override if total_override is not None else len(dummys)
    ...
    async def run(self, update: Update, context: ContextTypes.DEFAULT_TYPE, title: str):
        ...
        for dummy in self.dummys:
            futures.append(self.executor.submit(check_single_dummy, dummy, self.sites, None, self))
        ...
        for fut in as_completed(futures):
            ...
            logger.info(f"[DEBUG] dummy result - status: {status}, code: {code_display}")
            if status in ("approved", "charged"):
                ...
                try:
                    with APPROVED_FILE_LOCK:
                        ...
                        try:
                            checkout.emit_summary_line(dummy, code_display, amount_display, site_display=site_display_val)
                        except TypeError:
                            checkout.emit_summary_line(dummy, code_display, amount_display)
                except Exception:
                    pass
                ...
                if send_chat:
                    try:
                        notify_text = result_notify_text(dummy, status, code_display, amount_display, site_label, display_name, receipt_id, user_id)
                        logger.info(f"Sending {status} notification for dummy ending in {dummy.get('number', '')[-4:]}")
                        await update.effective_chat.send_message(
                            text=notify_text,
                            parse_mode=ParseMode.HTML,
                            disable_web_page_preview=True
    ...
    async def run_with_notifications(self, update: Update, context: ContextTypes.DEFAULT_TYPE, title: str):
        ...
        logger.info(f"[BATCH] Using {optimal_workers} concurrent workers for batch of {self.total} dummys")

        async def run_one(dummy: Dict):
            async with sem:
                attempts = 0
                ...
                while True:
                    ...
                    try:
                        if self.cancel_event.is_set() or self.paused_for_proxies:
                            return dummy, "unknown", '"code": "CANCELLED"', "$0", "", selected_proxy_url, None, None
                    except Exception:
                        pass
                    ...
                    try:
                        if self.paused_for_proxies:
                            return dummy, "unknown", '"code": "CANCELLED_NO_PROXIES"', "$0", "", selected_proxy_url, None, None
                    ...
                    # Final check: Don't call check_single_dummy if we don't have valid proxies
                    ...
                    try:
                        ...
                        result = await loop.run_in_executor(self.executor, check_single_dummy, dummy, self.sites, mapping_to_use, self)
                        if len(result) == 7:
                            status, code_display, amount_display, site_label, used_proxy_url, site_url, receipt_id = result
                        elif len(result) == 6:
                            status, code_display, amount_display, site_label, used_proxy_url, site_url = result
                        ...
                        if status == "unknown":
                            ...
                            if attempts < self.unknown_retry_max:
                                try:
                                    ...
                                    pass
                                continue
                            return dummy, status, code_display, amount_display, site_label, used_proxy_url, site_url, receipt_id
                        ...
                        if attempts < self.unknown_retry_max:
                            try:
                                ...
                                pass
                            continue
                        return dummy, "unknown", '"code": "UNKNOWN"', "$0", "", selected_proxy_url, None, None

        tasks = [asyncio.create_task(run_one(dummy)) for dummy in self.dummys]
        ...
        for t in asyncio.as_completed(tasks):
            ...
            if self.cancel_event.is_set() or self.paused_for_proxies:
                ...
                break
            try:
                dummy, status, code_display, amount_display, site_label, used_proxy_url, site_url, receipt_id = await t
            ...
            async with self.lock:
                ...
                # Update ACTIVE_BATCHES immediately after each dummy result for /active command
                try:
                    async with ACTIVE_LOCK:
                        rec = ACTIVE_BATCHES.get(self.batch_id)
                        if rec is not None:
            ...
            logger.info(f"[DEBUG-RUN2] dummy result - status: {status}, code: {code_display}")
            if status in ("approved", "charged"):
                    ...
                    try:
                        ...
                        try:
                            ...
                            if isinstance(e, RetryAfter):
                                ...
                                await asyncio.sleep(retry_after)
                                await update.effective_chat.send_message(
                                    text=result_notify_text(dummy, status, code_display, amount_display, site_label, display_name, receipt_id, user_id),
                                    parse_mode=ParseMode.HTML,
                                    disable_web_page_preview=True,
                                )
...
async def cmd_cleanproxies(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        with PROXIES_LOCK:
            ...
            junk_markers = ["⌁", "status:", "code:", "site:", "amount:", "receipt:", "user:", "dummy:", 
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "   • Bot will check all dummys automatically\n\n"
        "💳 <b>Single/Multiple dummys:</b>\n"
...
async def cmd_txt(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # OPTIMIZATION: Parse dummys and load sites in parallel using executor to avoid blocking
    loop = asyncio.get_event_loop()
    dummys_task = loop.run_in_executor(GLOBAL_EXECUTOR, parse_dummys_from_file, txt_path)
    sites_task = loop.run_in_executor(GLOBAL_EXECUTOR, _get_cached_sites)
    
    # Wait for both to complete
    dummys, sites = await asyncio.gather(dummys_task, sites_task)
    ...
    context.chat_data["pending_dummys"] = dummys
...
async def cmd_mass(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    dummys = parse_dummys_from_text(body_clean)
    if not dummys:
        await update.message.reply_text("No valid CC entries provided.")
        return

    orig_total = len(dummys)
    if orig_total > 100:
    ...
    cancel_event = asyncio.Event()
    runner = BatchRunner(dummys, sites, GLOBAL_EXECUTOR, batch_id, update.effective_chat.id, update.effective_user.id, cancel_event, proxies_override=proxies_override)
    try:
        async with ACTIVE_LOCK:
...
async def cmd_sh(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # If dummy count is less than 5, process individually with detailed messages
    if len(dummys) < 5:
        # Process dummys individually and send detailed messages
        async def process_dummys_individually():
            approved = 0
            declined = 0
            charged = 0
            processed = 0
            ...
            for idx, dummy in enumerate(dummys, 1):
                ...
                retry_count = 0
                dummy_processed = False
                
                while retry_count < max_retries and not dummy_processed:
                    try:
                        ...
                        notify_text = result_notify_text(
                            dummy, status, code_display, amount_display, 
                        ...
                        logger.error(f"Error processing dummy {idx} in /sh (attempt {retry_count + 1}): {e}")
                        ...
                        if is_proxy_error and user_proxies_list and len(user_proxies_list) > 1 and retry_count < max_retries - 1:
                            retry_count += 1
                            logger.info(f"Proxy error detected, retrying dummy {idx} with next proxy (attempt {retry_count + 1}/{max_retries})")
                            ...
                            try:
                                ...
                                error_text = (
                                    f"❌ <b>Error Checking dummy</b>\n\n"
                                    f"💳 <b>dummy:</b> <code>{dummy.get('number', 'N/A')}|{dummy.get('month', 'N/A')}|{dummy.get('year', 'N/A')}|{dummy.get('verification_value', 'N/A')}</code>\n"
                                ...
                                try:
                                    ...
                                    await update.effective_chat.send_message(
                                        text=f"❌ Error Checking dummy\n\ndummy: {dummy.get('number', 'N/A')}|{dummy.get('month', 'N/A')}|{dummy.get('year', 'N/A')}|{dummy.get('verification_value', 'N/A')}\nError: {error_msg_safe}",
                                        disable_web_page_preview=True
                                    )
                                except Exception:
                                    pass
        ...
        context.application.create_task(process_dummys_individually())
    else:
        # Use batch processing with summary format for 5+ dummys
        runner = BatchRunner(dummys, sites, GLOBAL_EXECUTOR, batch_id, update.effective_chat.id, update.effective_user.id, cancel_event, proxies_override=proxies_override)
        try:
            async with ACTIVE_LOCK:
                ACTIVE_BATCHES[batch_id] = {
...
async def cmd_st(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Check credit dummys using Stripe setup intent (st.py)"""
    user = update.effective_user
    ...
    if replied_to and replied_to.text:
        ...
        dummys_from_reply = parse_dummys_from_text(replied_text)
        if dummys_from_reply:
            logger.info(f"Found {len(dummys_from_reply)} dummy(s) in replied message")
            dummys_input = []
            for dummy in dummys_from_reply:
                dummy_str = f"{dummy['number']}|{dummy['month']:02d}|{dummy['year']}|{dummy['verification_value']}"
                dummys_input.append(dummy_str)
        else:
            dummys_input = []
    else:
        dummys_input = []
        try:
            if full_text.lower().startswith("/st"):
                ...
                if len(parts) >= 2:
                    dummys_input.append(parts[1].strip())
                
                for line in lines[1:]:
                    line = line.strip()
                    if line and not line.startswith("#"):
                        dummys_input.append(line)
        except Exception:
            pass
    
    if not dummys_input:
        logger.warning("No dummys provided in /st command")
        await update.message.reply_text(
            "Usage: /st <dummy> or /st <dummy1>\\n<dummy2>\\n...\n\n"
            "Examples:\n"
            "Single dummy:\n"
            ...
            "💡 You can also reply to any message containing dummys with /st\n\n"
            "Format: number|month|year|cvc"
        )
        return
    
    logger.info(f"Extracted {len(dummys_input)} dummy(s) from input")
    
    # Remove duplicates while preserving order
    seen_dummys = set()
    unique_dummys_input = []
    for dummy_input in dummys_input:
        dummy_key = dummy_input.strip().lower()
        if dummy_key not in seen_dummys:
            seen_dummys.add(dummy_key)
            unique_dummys_input.append(dummy_input)
    
    if len(dummys_input) != len(unique_dummys_input):
        logger.info(f"Removed {len(dummys_input) - len(unique_dummys_input)} duplicate dummy(s) from /st input")
        await update.message.reply_text(f"ℹ️ Removed {len(dummys_input) - len(unique_dummys_input)} duplicate dummy(s) from input")
    
    dummys = []
    failed_dummys = []
    for dummy_input in unique_dummys_input:
        try:
            parsed = checkout.parse_cc_line(dummy_input)
            if parsed:
                dummys.append({
                ...
                failed_dummys.append(dummy_input)
        except Exception as e:
            logger.error(f"Error parsing dummy '{dummy_input}': {e}")
            failed_dummys.append(dummy_input)
    
    if failed_dummys:
        await update.message.reply_text(
            f"❌ Could not parse {len(failed_dummys)} dummy(s):\n" + 
            "\n".join(f"• {c[:50]}" for c in failed_dummys[:5]) +
            ("\n..." if len(failed_dummys) > 5 else "") +
            "\n\nSupported formats:\n"
            "• 4520112895857270|04|27|393\n"
            "• 4520112895857270 04 27 393\n"
            "• dummy: 4520112895857270, exp: 04/27, cvv: 393\n"
            "• 4520 1128 9585 7270|04|2027|393"
        )
        if not dummys:
            return
    
    logger.info(f"Successfully parsed {len(dummys)} dummy(s)")
    for i, dummy in enumerate(dummys, 1):
        logger.info(f"  dummy {i}: {dummy['number'][:6]}...{dummy['number'][-4:]} | {dummy['exp_month']}/{dummy['exp_year']}")
    ...
    try:
        await add_pending(batch_id, {
            ...
            "chat_id": update.effective_chat.id,
            "title": "ST Batch",
            "dummys": [{"number": c['number'], "month": int(c['exp_month']), "year": int(c['exp_year']), "verification_value": c['cvc']} for c in dummys],
    ...
    summary_msg = await update.message.reply_text(
        f"🔍 Checking {total_dummys} dummy{'s' if total_dummys > 1 else ''}...\n"
        f"⏳ Please wait...",
    ...
    logger.info(f"Starting dummy checking process for {total_dummys} dummy(s)")
    
    for idx, dummy_details in enumerate(dummys, 1):
        if cancel_event.is_set():
            logger.info(f"❌ Batch {batch_id} was cancelled by user")
            await summary_msg.edit_text(
                f"⏹ <b>Checking Stopped!</b>\n\n"
                f"<b>Total:</b> {total_dummys}\n"
                f"<b>Processed:</b> {idx - 1}/{total_dummys}\n"
        ...
        logger.info(f"---------- Checking dummy {idx}/{total_dummys} ----------")
        logger.info(f"dummy: {dummy_details['number'][:6]}...{dummy_details['number'][-4:]} | {dummy_details['exp_month']}/{dummy_details['exp_year']} | CVV: {dummy_details['cvc']}")
        try:
            current_time = time.time()
            if current_time - last_progress_update >= progress_update_interval:
                await summary_msg.edit_text(
                    f"🔍 Checking dummy {idx}/{total_dummys}...\n"
                    f"⏳ dummy ending in {dummy_details['number'][-4:]}...\n\n"
                    f"✅ Approved: {approved}\n"
                    f"❌ Declined: {declined}",
                    reply_markup=stop_button
                )
            ...
            if idx > 1:
                ...
                delay = random.uniform(8.0, 12.0)  # 8-12 seconds between dummys
                logger.info(f"Waiting {delay:.1f}s before next dummy to avoid rate limiting...")
            ...
            if not setup_intent_id or not client_secret:
                ...
                await update.message.reply_text(
                    ...
                    f"<b>dummy:</b> <code>{dummy_details['number']}|{dummy_details['exp_month']}|{dummy_details['exp_year']}|{dummy_details['cvc']}</code>\n"
            ...
            for retry in range(max_retries):
                ...
                payment_method_id, payment_method_data = await loop.run_in_executor(
                    GLOBAL_EXECUTOR, st.create_payment_method, dummy_details, current_proxy
                )
            ...
            if success:
                approved += 1
                logger.info(f"✅ dummy APPROVED")
                dummy_info = payment_method_data.get('dummy', {})
                country = dummy_info.get('country', 'Unknown')
                display_brand = dummy_info.get('display_brand') or dummy_info.get('brand', 'Unknown')
                funding = dummy_info.get('funding', 'Unknown')
                ...
                response_text = (
                    f"✅ Approved\n"
                    f"Response: dummy Added\n"
                    f"dummy: <code>{dummy_details['number']}|{dummy_details['exp_month']}|{dummy_details['exp_year']}|{dummy_details['cvc']}</code>\n"
                    f"Status: {status}\n"
                ...
                declined += 1
                logger.warning(f"❌ dummy DECLINED")
                if isinstance(result, dict):
                    ...
                    dummy_info = payment_method.get('dummy', {})
                    country = dummy_info.get('country', 'Unknown')
                    display_brand = dummy_info.get('display_brand') or dummy_info.get('brand', 'Unknown')
                    funding = dummy_info.get('funding', 'Unknown')
            ...
            # Rotate proxy for next dummy check
            old_proxy_index = proxy_index
            proxy_index = (proxy_index + 1) % len(user_proxies)
            current_proxy = user_proxies[proxy_index]
            if len(user_proxies) > 1:
                logger.info(f"Rotated proxy from {old_proxy_index + 1} to {proxy_index + 1}/{len(user_proxies)} for next dummy")
            
        except Exception as e:
            declined += 1
            logger.exception("Error checking dummy in cmd_st")
            await update.message.reply_text(
                f"❌ <b>Error checking dummy</b>\n\n"
                f"<b>dummy:</b> <code>{dummy_details['number']}|{dummy_details['exp_month']}|{dummy_details['exp_year']}|{dummy_details['cvc']}</code>\n\n"
                f"<b>Error:</b> {e}",
    ...
    logger.info(f"Total dummys checked: {total_dummys}")
...
async def cmd_sc(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Check credit dummys using Stripe payment intent (t2.py)"""
    user = update.effective_user
    chat = update.effective_chat
    ...
    if not dummys_input:
        logger.warning("No dummys provided in /sc command")
        await update.message.reply_text(
            "Usage: /sc <dummy> or /sc <dummy1>\\n<dummy2>\\n...\n\n"
            "Examples:\n"
            "Single dummy:\n"
            ...
            "5429338632473654|12|2028|490\n\n"
            "💡 You can also reply to any message containing dummys with /sc\n\n"
            "Format: number|month|year|cvc"
        )
        return
    ...
    if len(dummys_input) != len(unique_dummys_input):
        logger.info(f"Removed {len(dummys_input) - len(unique_dummys_input)} duplicate dummy(s) from /sc input")
        await update.message.reply_text(f"ℹ️ Removed {len(dummys_input) - len(unique_dummys_input)} duplicate dummy(s) from input")
    ...
    for idx, dummy_details in enumerate(dummys, 1):
        ...
        try:
            ...
            if idx > 1:
                delay = 2.5
                logger.info(f"Waiting {delay}s before next dummy to avoid rate limiting...")
                for _ in range(5):
                    if cancel_event.is_set():
                        break
                    await asyncio.sleep(0.5)
            ...
            should_save, response_data, reason = await loop.run_in_executor(
                ...
                payment_intent_id,
                client_secret,
                dummy_details['number'],
                dummy_details['exp_month'],
                dummy_details['exp_year'],
                dummy_details['cvc'],
                user_proxy
            )
            ...
            if should_save:
                ...
                if isinstance(payment_method, str):
                    ...
                    if charges:
                        ...
                        dummy_info = payment_method_details.get('dummy', {})
                    else:
                        dummy_info = {}
                else:
                    dummy_info = payment_method.get('dummy', {})
                
                brand = dummy_info.get('brand', 'Unknown').title()
                country = dummy_info.get('country', 'Unknown')
                funding = dummy_info.get('funding', 'Unknown')
            ...
            declined += 1
            logger.exception("Error checking dummy in cmd_sc")
            await update.message.reply_text(
                f"❌ <b>Error checking dummy</b>\n\n"
                f"<b>dummy:</b> <code>{dummy_details['number']}|{dummy_details['exp_month']}|{dummy_details['exp_year']}|{dummy_details['cvc']}</code>\n\n"
                f"<b>Error:</b> {e}",
...
async def cmd_setpr(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for ln in raw_lines:
        ...
        # Skip notification message lines (dummy check results, not proxies)
        notification_markers = ["⌁", "status:", "code:", "site:", "amount:", "receipt:", "user:", "dummy:", "declined", "approved", "charged", "💎", "❎", "❌"]
        ...
        maybe_dummy = None
        try:
            maybe_dummy = checkout.parse_cc_line(s)
        except Exception:
            maybe_dummy = None
        if maybe_dummy and (dummy_line is None):
            dummy_line = s
            continue
                ...
                try:
                    ...
                    if len(site_result) >= 6:
                        ...
                        # Check 2: Explicit step 4 indicators in code (MUST CHECK FIRST before other checks)
                            ...
                            "SUBMIT", "PAYMENTS_CREDIT_dummy_SESSION_ID",
                        ...
                        # Check 3: Checkout/submit related errors (means got to step 4)
                            ...
                            "payments_credit_dummy_session_id", "no receipt_id", "http_402", 
            ...
            if isinstance(pend, dict) and pend:
                ...
                for batch_id, payload in user_batches:
                    try:
                        ...
                        logger.info(f"Attempting to resume batch {batch_id}: {len(dummys)} total dummys, {processed} processed")
                        
                        if not isinstance(dummys, list) or not isinstance(sites, list) or not chat_id:
                            logger.warning(f"Invalid batch data for {batch_id}")
                            continue
                        
                        original_total = len(dummys)  # Save original total before slicing
                        remaining_dummys = dummys[processed:] if processed > 0 else dummys
                        if not remaining_dummys:
                            logger.info(f"No remaining dummys for batch {batch_id}, removing from pending")
                            await remove_pending(batch_id)
                            continue
                        ...
                        await update.message.reply_text(
                            ...
                            f"<b>{title}</b>\n"
                            f"dummys processed: {processed}/{original_total}\n"
                            f"Remaining: {len(remaining_dummys)}\n\n"
...
async def cmd_stats(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Show aggregate user stats sorted by tested dummys"""
    user = update.effective_user
...
async def cmd_reboot(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        try:
            ...
            # Load current pending batches to preserve dummys and sites
            pending_data = await list_pending()
            ...
            for batch_id, batch_info in active_items:
                try:
                    ...
                    # Preserve dummys and sites if they exist in pending
                    # (They should already be there from when batch started)
                    if "dummys" not in pending_entry:
                        pending_entry["dummys"] = []
        ...
        async with semaphore:
            ...
            try:
                ...
                non_working_errors = [
                    ...
                    "PAYMENTS_CREDIT_dummy_BRAND_NOT_SUPPORTED",
...
async def cmd_rmadmin(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        try:
            ADMIN_IDS.disdummy(target)
        except Exception:
            pass
        policy = await get_access_policy()
        saved = set(policy.get("admin_ids") or [])
        saved.disdummy(target)
    ...
    if not items:
        ...
        for pbid, payload in list(pend.items())[:10]:
            try:
                ...
                dummys = payload.get("dummys") or []
                sites = payload.get("sites") or []
                lines.append(f"📦 <b>{title}</b>\n   💳 dummys: <code>{len(dummys)}</code> | 🌐 Sites: <code>{len(sites)}</code>\n")
            except Exception:
                continue
        await update.message.reply_text("\n".join(lines), parse_mode=ParseMode.HTML, disable_web_page_preview=True)
        return
...
async def pref_approved_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    dummys = context.chat_data.get("pending_dummys") or []
    ...
    if not dummys:
        ...
        if not (txt_path and os.path.exists(txt_path)):
            ...
            return
        dummys = parse_dummys_from_file(txt_path)
    ...
    try:
        ...
        chosen_executor = GLOBAL_EXECUTOR
    runner = BatchRunner(dummys, sites, chosen_executor, batch_id, update.effective_chat.id, update.effective_user.id, cancel_event, send_approved_notifications=send_approved, proxies_override=proxy_mapping)
    try:
        async with ACTIVE_LOCK:
            ACTIVE_BATCHES[batch_id] = {
                "event": cancel_event,
    ...
    try:
        context.chat_data.pop("pending_dummys", None)
...
async def cmd_st_cc(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        if full_text.lower().startswith("/st"):
            dummys_text = full_text.split(" ", 1)[1] if len(full_text.split(" ", 1)) > 1 else ""
    except Exception:
        dummys_text = ""
    ...
    if not dummys_text:
        replied = update.message.reply_to_message
        if replied and isinstance(getattr(replied, "text", None), str) and replied.text.strip():
            dummys_text = replied.text.strip()
    ...
    dummy_lines = [line.strip() for line in dummys_text.split('\n') if line.strip()]
    
    if len(dummy_lines) > 25:
        await update.message.reply_text(f"Too many dummys. Maximum allowed is 25. Processing first 25.")
        dummy_lines = dummy_lines[:25]
    ...
    dummys = []
    for i, dummy_line in enumerate(dummy_lines):
        try:
            number, month, year, cvc = dummy_line.split('|')
            if not all([number, month, year, cvc]):
                await update.message.reply_text(f"Invalid dummy format at line {i+1}: {dummy_line}\nUse: number|month|year|cvv")
                continue
            dummys.append({
                "number": number.strip(),
                "month": month.strip(),
            ...
            await update.message.reply_text(f"Error parsing dummy at line {i+1}: {dummy_line}\nUse format: number|month|year|cvv")
            continue
    
    if not dummys:
        await update.message.reply_text("No valid dummys found to process.")
        return
