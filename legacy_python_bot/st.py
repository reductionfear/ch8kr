...
def save_approved_dummy(dummy_details):
    try:
        with open('approved.txt', 'a', encoding='utf-8') as file:
            dummy_string = f"{dummy_details['number']}|{dummy_details['exp_month']}|{dummy_details['exp_year']}|{dummy_details['cvc']} Status: Approved\n"
            file.write(dummy_string)
    
    except Exception as e:
        print(f"Error saving approved dummy: {e}")
...
def create_payment_method(dummy_details, proxy=None):
    headers = {
    ...
    data = {
        ...
        'dummy[number]': dummy_details['number'],
        'dummy[cvc]': dummy_details['cvc'],
        'dummy[exp_month]': dummy_details['exp_month'],
        'dummy[exp_year]': dummy_details['exp_year'],
        'pasted_fields': 'number',
        'payment_user_agent': 'stripe.js/2016dc44bd; stripe-js-v3/2016dc44bd; split-dummy-element',
        ...
        'client_attribution_metadata[merchant_integration_subtype]': 'split-dummy-element',
    ...
    try:
        print(f"Creating payment method for dummy ending in {dummy_details['number'][-4:]}...")
...
def confirm_setup_intent(setup_intent_id, client_secret, payment_method_id, payment_method_data, dummy_details, proxy=None):
    headers = {
        'accept': 'application/json',
        'accept-language': 'en-GB,en;q=0.9',
        'content-type': 'application/x-www-form-urlencoded',
    ...
    try:
        print(f"Confirming setup intent for dummy ending in {dummy_details['number'][-4:]}...")
        encoded_payload = urllib.parse.urlencode(data)
        ...
        if response.status_code == 429:
            ...
            dummy = payment_method_data.get('dummy', {})
            country = dummy.get('country')
            display_brand = dummy.get('display_brand')
            brand = dummy.get('brand')
            funding = dummy.get('funding')
            
            print(f"Success! Brand: {display_brand or brand}, Country: {country}, Type: {funding}")
            save_approved_dummy(dummy_details)
            ...
            dummy = payment_method.get('dummy', {})
            country = dummy.get('country')
            display_brand = dummy.get('display_brand')
            brand = dummy.get('brand')
            funding = dummy.get('funding')
...
def check_dummy(dummy_details):
    print(f"\n{'='*50}")
    print(f"Checking dummy: {dummy_details['number']}|{dummy_details['exp_month']}|{dummy_details['exp_year']}|{dummy_details['cvc']}")
    print(f"{'='*50}")
    ...
    payment_method_id, payment_method_data = create_payment_method(dummy_details)
...
if __name__ == "__main__":
    ...
    if not dummys:
        print("No dummys found")
    else:
        print(f"Found {len(dummys)} dummys")
        approved_count = 0
        for i, dummy in enumerate(dummys, 1):
            print(f"\ndummy {i}/{len(dummys)}")
            if check_dummy(dummy):
                approved_count += 1
        
        print(f"\n{'='*50}")
        print(f"Done: {approved_count}/{len(dummys)} approved")
        print(f"{'='*50}")